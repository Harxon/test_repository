...
#if 1
	msg->storage_no[snum].storage_status = buf_msg->snum - 48;
#endif
	//判断刷新类型
	//刷新环境信息
	if('e' == buf_msg->type){
		puts("fflush env info...");
		msg->storage_no[snum].temperature = (buf_msg->temp[1] - 48)/256;
		msg->storage_no[snum].temperature += buf_msg->temp[0] - 48;

		msg->storage_no[snum].humidity = (buf_msg->hum[1] - 48)/256;
		msg->storage_no[snum].humidity += buf_msg->hum[0] - 48;

		msg->storage_no[snum].illumination = buf_msg->lux;

		msg->storage_no[snum].x = buf_msg->x - 48;
		msg->storage_no[snum].y = buf_msg->y - 48;
		msg->storage_no[snum].z = buf_msg->z - 48;

		msg->storage_no[snum].battery = buf_msg->bat;
		msg->storage_no[snum].adc = buf_msg->adc;
